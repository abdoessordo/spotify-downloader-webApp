...
/**
 * Checks if a user is logged in based on whether or not there is an access token stored in localStorage
 * @returns {boolean} Whether or not the user is logged in
 */
export const isLoggedIn = (): boolean => {
  return LOCALSTORAGE_VALUES.accessToken && !hasTokenExpired() ? true : false;
};
...
export const getAccessToken = (): string | boolean => {
  ...
  if (queryParams[LOCALSTORAGE_KEYS.accessToken]) {
    ...
    for (const property in queryParams) {
      window.localStorage.setItem(
        property,
        queryParams[property as keyof typeof queryParams]!
      );
    }
    // Set timestamp
    window.localStorage.setItem(
...
/**
 * Get the Spotify user profile from the Spotify Web API
...
/**
 * Get the user's playlists from the Spotify Web API
